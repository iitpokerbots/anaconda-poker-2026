...
BOT_1_NAME = 'BotA'
BOT_1_FILENAME = 'example_bot.py'

BOT_2_NAME = 'BotB'
BOT_2_FILENAME= 'example_bot.py'
```
...
ActionPass(<list-of-indices>)       # Bid amount for auction
```

The pass logic of your strategy needs to be written in this function, for example:

```python
if current_state.street == 'TriplePass':
    return ActionPass([0, 1, 2]) # always pass first 3
```
...
if current_state.street == 'DoublePass':
    return ActionPass([0, 1]) # always pass first 2

if current_state.street == 'SinglePass':
    return ActionPass([0]) # always pass first card
...
Streets are denoted by strings in this system. The different possible streets are `TriplePass`, `DoublePass`, `SinglePass`, `Betting#1`, `Betting#2`, `Betting#3`. The current street can be accessed via `current_state.street`.
